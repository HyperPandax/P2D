...
private:
	...
	glm::vec2  _mousePos; /// @brief Point2 of floats containing Mouse Position
...
public:
	...
	static bool GetKeyDown(int keyCode) { return GetInstance()->_keys[keyCode] && !GetInstance()->_keysLast[keyCode]; };

	// Returns true if Key is down this frame and last frame.
	static bool GetKey(int keyCode) { return GetInstance()->_keys[keyCode] && GetInstance()->_keysLast[keyCode]; };

	// Returns true if Key is down last frame, but not down this frame
	static bool GetKeyUp(int keyCode) { return !GetInstance()->_keys[keyCode] && GetInstance()->_keysLast[keyCode]; };
	
	// Sets a key
	static void SetKey(int keyCode, bool state);

	// Returns true if Key is down this frame, but not down last frame.
	static bool GetButtonDown(int buttonCode) { return GetInstance()->_buttons[buttonCode] && !GetInstance()->_buttonsLast[buttonCode]; };
	
	// Returns true if Key is down this frame and last frame.
	static bool GetButton(int buttonCode) { return GetInstance()->_buttons[buttonCode] && GetInstance()->_buttonsLast[buttonCode]; };
	
	// Returns true if Key is down last frame, but not down this frame
	static bool GetButtonUp(int buttonCode) { return !GetInstance()->_buttons[buttonCode] && GetInstance()->_buttonsLast[buttonCode]; };
	
	// Sets a key
	static void SetButton(int buttonCode, bool state) { Input::GetInstance()->_buttons[buttonCode] = state; };

	// Sets the mouse position
	static void SetMousePos(glm::vec2 point) { Input::GetInstance()->_mousePos = point; };

	// Returns the mouse position as a Point2f
	static glm::vec2 GetMousePosition() { return Input::GetInstance()->_mousePos; };

	// Returns the last key pressed this frame, if there is no key pressed this frame it returns KEYCODE_EMPTY_KEY
	static int GetLastKey() { return Input::GetInstance()->lastKey; };
};
